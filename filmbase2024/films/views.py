...
from .models import Country, Film, FilmCrew, Genre, Person
from .forms import CountryForm, CrewForm, GenreForm, FilmForm, PersonForm
...
def film_detail(request, id):
    queryset = Film.objects.prefetch_related("country", "genres", "crew")
...
@user_passes_test(check_admin)
def crew_create(request, id):
    film = get_object_or_404(Film, id=id)
    if request.method == 'POST':
        form = CrewForm(request.POST, request.FILES, initial={'film': film})
        if form.is_valid():
            form.save()
            messages.success(request, 'Съемочная группа создана')
            return redirect('films:film_detail', id=film.id)
    else:
        form = CrewForm(instance=film, initial={'film': film, 'name': None})
    return render(request, 'films/film/crew_create.html',
                  {'form': form})

@user_passes_test(check_admin)
def crew_update(request, id):
    crew = get_object_or_404(FilmCrew, id=id)
    if request.method == 'POST':
        form = CrewForm(request.POST, request.FILES, instance=crew)
        if form.is_valid():
            form.save()
            messages.success(request, 'Съемочная группа изменена')
            return redirect('films:film_detail', id=crew.film.id)
    else:
        form = CrewForm(instance=crew)
    return render(request, 'films/film/crew_update.html',
                  {'form': form})

@user_passes_test(check_admin)
def crew_delete(request, id):
    crew = get_object_or_404(FilmCrew, id=id)
    film = get_object_or_404(Film, id=crew.film.id)
    if request.method == 'POST':
        crew.delete()
        messages.success(request, 'Съемочная группа удалена')
        return redirect('films:film_detail', id=film.id)
    return render(request, 'films/film/crew_delete.html',
                  {'crew': crew})
...
class PersonAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        crew = Person.objects.all()
        if self.q:
            crew = crew.filter(name__istartswith=self.q)
        return crew
...
class GenreAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        genres = Genre.objects.all()
        if self.q:
            genres = genres.filter(name__istartswith=self.q)
        return genres
